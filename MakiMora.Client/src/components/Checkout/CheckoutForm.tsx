...
import type { CartItem, CreateOrderRequest } from '../../DTOs';
...
const CheckoutForm: React.FC = () => {
  const [formData, setFormData] = useState({
    ...
    customerAddress: '',
    comment: '',
    locationId: '11111111-1111-1111-1111-11111111' // Default location
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Calculate delivery fee (could be based on location or other factors)
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              {loading ? (
                ...
                `Оформить заказ на ${(state.items.reduce((sum, item) => sum + (item.productPrice * item.quantity), 0) + 200).toFixed(2)} ₽`
              )}
            </button>
          </form>
        </div>
          ...
          <div className="border border-gray-200 rounded-md p-4">
            ...
            <div className="mt-6 border-t border-gray-200 pt-4">
              <div className="flex justify-between text-base font-medium text-gray-900 mb-1">
                <p>Товары</p>
                <p>{state.items.reduce((sum, item) => sum + (item.productPrice * item.quantity), 0).toFixed(2)} ₽</p>
              </div>
              ...
              <div className="flex justify-between text-lg font-bold text-gray-900 mt-4 pt-4 border-t border-gray-200">
                <p>Итого</p>
                <p>{(state.items.reduce((sum, item) => sum + (item.productPrice * item.quantity), 0) + 200).toFixed(2)} ₽</p>
              </div>
            </div>
          </div>
        </div>
